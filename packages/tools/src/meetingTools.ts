...
function formatMeeting(meeting: Meeting): {
  ...
  if (
    meeting.url &&
    (meeting.url.includes("zoom") ||
      meeting.url.includes("meet.google") ||
      meeting.url.includes("teams"))
  ) {
    format = "online";
...
- Filter by format (online, in-person, or both)`,
  inputSchema: z.object({
    fellowship: z
      .enum(["aa", "na", "cma", "rd", "all"])
      .describe(
        "Fellowship type to search for (AA=Alcoholics Anonymous, NA=Narcotics Anonymous, CMA=Crystal Meth Anonymous, RD=Recover Dharma)",
      ),
    when: z
      .enum([
        "now",
      ...
      .default("now")
      .describe(
        'When to search for meetings: "now" for live meetings, or a day for scheduled meetings',
      ),
    format: z
    ...
    timezone: z
      ...
      .describe(
        "IANA timezone for schedule queries (e.g., America/New_York, America/Los_Angeles)",
      ),
  }),
      ...
      try {
        ...
        if (when === "now") {
          ...
          if (dayOfWeek !== undefined) {
            const response = await client.getMeetingsForDay(
              dayOfWeek,
              timezone,
            );
            meetings = response.meetings;
        ...
        logger.info(
          { count: formattedMeetings.length, source },
          "Meetings found",
        );
...
export const getLiveMeetings = tool({
  description:
    "Get recovery meetings that are happening RIGHT NOW. Returns all live meetings across all fellowships.",
  inputSchema: z.object({
    fellowship: z
      .enum(["aa", "na", "cma", "rd", "all"])
      .default("all")
