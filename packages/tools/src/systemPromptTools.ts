 ...
 */
type RefreshBaseIdentityFn = () => Promise<{
  content: string;
  id: string;
  name: string;
} | null>;
...
export const refreshSystemPrompt = tool({
  description:
    "Refresh your system prompt/base identity from the database. Use this when asked to update your personality or instructions. Note: The updated prompt will take effect on the NEXT message, not the current one.",
  inputSchema: z.object({
    reason: z
      .string()
      .optional()
      .describe("Optional reason for refreshing the prompt"),
  }),
    ...
    return withSpan("tool.refreshSystemPrompt", async () => {
      ...
      if (!refreshBaseIdentityFn) {
        ...
        return {
          success: false,
          message:
            "System prompt refresh is not configured. Please contact an administrator.",
        };
      }
      ...
      try {
        ...
        if (result) {
          logger.info(
            { promptId: result.id, promptName: result.name },
            "System prompt refreshed successfully",
          );
          return {
            success: true,
        ...
        return {
          success: false,
          message:
            "Failed to refresh system prompt due to a database error. Please try again later.",
        };
      }
    });
  },
...
export const clearConversation = tool({
  description:
    "Clear all memory for the current conversation. Use this when the user asks to start fresh, forget everything, or clear the conversation history. This will remove cached messages and context from all memory tiers.",
  inputSchema: z.object({
    reason: z
      .string()
      .optional()
    ...
    return withSpan("tool.clearConversation", async () => {
      ...
      if (!clearConversationFn) {
        ...
        return {
          success: false,
          message:
            "Clear conversation is not configured. Please contact an administrator.",
        };
      }
      ...
      if (!getConversationIdFn) {
        ...
        return {
          success: false,
          message:
            "Cannot determine current conversation. Please contact an administrator.",
        };
      }
      ...
      try {
        ...
        return {
          success: false,
          message:
            "An error occurred while clearing the conversation. Please try again later.",
        };
      }
    });
  },
