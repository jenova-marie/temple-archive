...
import { MedicalAdviceDetector } from "../../src/detectors/MedicalAdviceDetector.js";
  ...
  describe("preFilter", () => {
    ...
    describe("dosage patterns", () => {
      it("should detect mg dosage recommendation", () => {
        ...
        expect(matches.some((m) => m.pattern === "dosage_recommendation")).toBe(
          true,
        );
      });
      ...
      it("should detect frequency recommendation", () => {
        ...
        expect(matches.length).toBeGreaterThan(0);
        expect(matches.some((m) => m.pattern === "dosage_frequency")).toBe(
          true,
        );
      });
    });
    ...
    describe("diagnostic patterns", () => {
      it("should detect disorder diagnosis", () => {
        ...
        expect(matches.length).toBeGreaterThan(0);
        expect(matches.some((m) => m.pattern === "diagnosis_statement")).toBe(
          true,
        );
      });
      ...
      it("should detect implied diagnosis", () => {
        ...
        expect(matches.length).toBeGreaterThan(0);
        expect(matches.some((m) => m.pattern === "diagnosis_sounds_like")).toBe(
          true,
        );
      });
      ...
      it("should detect probable diagnosis", () => {
        const matches = detector.preFilter(
          "You probably have a sleep disorder",
        );
        expect(matches.length).toBeGreaterThan(0);
        expect(matches.some((m) => m.pattern === "diagnosis_statement")).toBe(
          true,
    ...
    describe("treatment patterns", () => {
      it("should detect treatment instruction", () => {
        ...
        expect(matches.length).toBeGreaterThan(0);
        expect(matches.some((m) => m.pattern === "treatment_instruction")).toBe(
          true,
        );
      });
      ...
      it("should detect procedure recommendation", () => {
        ...
        expect(matches.length).toBeGreaterThan(0);
        expect(matches.some((m) => m.pattern === "medical_procedure")).toBe(
          true,
        );
      });
    });
    ...
    describe("no matches", () => {
      it("should not match general wellness advice", () => {
        const matches = detector.preFilter(
          "Try to get more sleep and exercise regularly",
        );
      ...
      it("should not match recovery encouragement", () => {
        const matches = detector.preFilter(
          "Your recovery journey is going well",
        );
        expect(matches).toHaveLength(0);
      });
