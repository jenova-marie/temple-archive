...
import { StubSafetyValidator } from './StubSafetyValidator.js'
import type { TraceContext, AssembledContext, SafetyViolation } from '@recoverysky/types'
...
describe('StubSafetyValidator', () => {
  ...
  describe('validate', () => {
    describe('passing validation', () => {
      it('returns passed=true for safe content', async () => {
        const output = 'I hear you. Recovery is a journey, and I am here to support you.'
    ...
    describe('PII detection', () => {
      ...
      it('does not flag partial SSN patterns', async () => {
        ...
        if (result.ok) {
          expect(result.value.passed).toBe(true)
          expect(result.value.violations.filter(v => v.type === 'pii')).toHaveLength(0)
        }
      })
    })
  ...
  describe('mock violations', () => {
    ...
    it('combines mock and detected violations', async () => {
      ...
      const result = await validator.validate('SSN: 123-45-6789', mockContext, ctx)
    ...
    it('accumulates multiple mock violations', async () => {
      validator.addMockViolation({ type: 'type1', severity: 'low', description: 'First' })
      validator.addMockViolation({ type: 'type2', severity: 'medium', description: 'Second' })
