...
import { EnablingDetector } from './EnablingDetector.js'
...
describe('EnablingDetector', () => {
  ...
  describe('preFilter', () => {
    ...
    describe('glorification patterns', () => {
      it('should detect deserve a drink', () => {
        const matches = detector.preFilter('You deserve a drink after all that')
      ...
      it('should detect romanticizing past use', () => {
        const matches = detector.preFilter('Remember the good times when we used to party')
        expect(matches.length).toBeGreaterThan(0)
        expect(matches[0].type).toBe('glorification')
      })
    })
    ...
    describe('normalization patterns', () => {
      ...
      it('should detect normal to want', () => {
        const matches = detector.preFilter("It's normal to want a drink sometimes")
        expect(matches.length).toBeGreaterThan(0)
        expect(matches[0].type).toBe('normalization')
      })
    ...
    describe('dampeners', () => {
      it('should reduce confidence when discussing past beliefs', () => {
        const text = "I used to think one drink wouldn't hurt, but now I know better"
      ...
      it('should reduce confidence with recovery first statement', () => {
        const text = "Some say moderation is fine, but your recovery comes first"
        const matches = detector.preFilter(text)
        if (matches.length > 0) {
          expect(matches[0].confidence).toBeLessThan(0.5)
        }
      ...
      it('should reduce confidence when correcting misconception', () => {
        const text = "People sometimes think everyone does it, but that's not true"
        const matches = detector.preFilter(text)
        if (matches.length > 0) {
          expect(matches[0].confidence).toBeLessThan(0.5)
        }
    ...
    describe('no matches', () => {
      it('should not match supportive recovery language', () => {
        const matches = detector.preFilter("I'm proud of your sobriety journey")
        expect(matches).toHaveLength(0)
      })

      it('should not match cravings acknowledgment without permission', () => {
        const matches = detector.preFilter('I understand you have cravings. Let us work through this together.')
        expect(matches).toHaveLength(0)
      })

      it('should not match relapse discussion without enabling', () => {
        const matches = detector.preFilter('Relapse can be part of recovery, but it is not inevitable')
        expect(matches).toHaveLength(0)
      })
    })
  })
  ...
  describe('analyzeLLM', () => {
    ...
    it('should parse LLM response correctly', async () => {
      ...
      const result = await detector.analyzeLLM('You deserve a drink', createTraceContext())
    ...
    it('should handle non-enabling response', async () => {
      ...
      const detector = new EnablingDetector(mockClient)
      const result = await detector.analyzeLLM("Keep up the great work!", createTraceContext())
