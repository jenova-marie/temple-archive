...
// Mock api module that src/commands/health.ts imports
vi.mock("../../src/api.js", () => ({
  checkHealth: vi.fn(),
  getMetrics: vi.fn(),
}));

// Mock config module that src/commands/health.ts imports
...
// Import after mocks
import { healthCommand, metricsCommand } from "../../src/commands/health.js";
import { checkHealth, getMetrics } from "../../src/api.js";
...
describe("health commands", () => {
  ...
  describe("healthCommand", () => {
    it("starts spinner when checking health", async () => {
      (checkHealth as Mock).mockResolvedValue({
        status: "healthy",
    ...
    it("formats uptime with hours and minutes", async () => {
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith(
        expect.stringContaining("2h 1m 5s"),
      );
    });

    it("handles health check errors", async () => {
      (checkHealth as Mock).mockRejectedValue(
  ...
  describe("metricsCommand", () => {
    ...
    it("displays metrics text", async () => {
      const metricsText =
        "# HELP recoverysky_uptime_seconds Uptime\nrecoverysky_uptime_seconds 12345";
      (getMetrics as Mock).mockResolvedValue(metricsText);
    ...
    it("handles metrics fetch errors", async () => {
      (getMetrics as Mock).mockRejectedValue(
