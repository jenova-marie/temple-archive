...
describe("MockAgentProvider", () => {
  ...
  describe("generate", () => {
    ...
    it("calculates token usage based on message length", async () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value.usage.inputTokens).toBe(
          Math.ceil(input.userMessage.length / 4),
        );
        expect(result.value.usage.outputTokens).toBe(
          Math.ceil(result.value.content.length / 4),
        );
      }
    });
    ...
    describe("contextual responses", () => {
      ...
      it("returns relapse response for relapse messages", async () => {
        const input = createAgentInput({
          userMessage: "I had a relapse yesterday",
        });
        const result = await provider.generate(input, ctx);
      ...
      it("returns meeting response for AA/NA requests", async () => {
        const input = createAgentInput({
          userMessage: "Can you find an AA meeting?",
        });
        const result = await provider.generate(input, ctx);
