...
import { EntityExtractor, DEFAULT_EXTRACTOR_CONFIG } from './EntityExtractor.js'
import type { Message, TraceContext, IKnowledgeStore } from '@recoverysky/types'
...
describe('EntityExtractor', () => {
  ...
  describe('extract - LLM parsing', () => {
    it('should parse entities from LLM response', async () => {
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              entities: [
                { name: 'John', type: 'person', importance: 0.8, context: 'Sponsor' },
                { name: 'work stress', type: 'trigger', importance: 0.6, context: 'Job related' },
              ],
              relationships: [
                { from: 'John', to: 'work stress', type: 'helps_with', strength: 0.7 },
              ],
            }),
          },
        ],
      ...
      const result = await extractor.extract(
        createMockMessage('user', 'I talked to my sponsor John about work stress'),
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              entities: [
                { name: 'John', type: 'person', importance: 0.8, context: 'High' },
                { name: 'weather', type: 'event', importance: 0.1, context: 'Low' },
              ],
              relationships: [],
            }),
          },
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              entities: [
                { name: 'John', type: 'person', importance: 0.8, context: 'Sponsor' },
                { name: 'happy', type: 'emotion', importance: 0.9, context: 'Feeling' },
              ],
              relationships: [],
            }),
          },
  ...
  describe('extract - error handling', () => {
    it('should return empty result when no client', async () => {
      const extractor = new EntityExtractor(null, mockKnowledgeStore, { mode: 'all' })
        ...
        content: [
          {
            type: 'text',
            text: JSON.stringify({
              entities: [{ name: 'John', type: 'person', importance: 0.8, context: 'Sponsor' }],
              relationships: [],
            }),
          },
        ],
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              entities: [
                { name: 'John', type: 'person', importance: 0.8, context: 'Sponsor' },
                { name: 'stress', type: 'trigger', importance: 0.7, context: 'Trigger' },
              ],
              relationships: [{ from: 'John', to: 'stress', type: 'helps_with', strength: 0.6 }],
            }),
          },
        ],
      })
