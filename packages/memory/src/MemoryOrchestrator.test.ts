...
import { MemoryOrchestrator, type MemoryOrchestratorConfig } from './MemoryOrchestrator.js'
import type {
  IContextStore,
  ISessionStore,
  IKnowledgeStore,
...
function createMockL1Store(overrides: Partial<IContextStore> = {}): IContextStore {
  return {
...
function createMockL2Store(overrides: Partial<ISessionStore> = {}): ISessionStore {
  return {
    ...
    getOrCreateUser: vi.fn().mockResolvedValue(ok({ id: 'user-id', externalId: 'ext-1', createdAt: new Date() })),
    ...overrides,
  }
}

function createMockL3Store(overrides: Partial<IKnowledgeStore> = {}): IKnowledgeStore {
  return {
...
describe('MemoryOrchestrator', () => {
  ...
  describe('retrieveContext', () => {
    describe('L1 cache hit', () => {
      it('returns from L1 when messages are cached', async () => {
        ...
        const result = await orchestrator.retrieveContext('conv-1', 'user-1', null, ctx)
    ...
    describe('L2 failure', () => {
      it('returns error when L2 fails', async () => {
        l1.getRecentMessages = vi.fn().mockResolvedValue(ok([]))
        l2.getConversationHistory = vi.fn().mockResolvedValue(
          err({ kind: 'ConnectionError' as const, message: 'DB error', context: {} })
        )
    ...
    describe('L4 semantic search', () => {
      it('falls back to L4 when L1 and L2 are empty', async () => {
        ...
        const result = await orchestrator.retrieveContext('conv-1', 'user-1', embedding, ctx)
    ...
    describe('empty context', () => {
      it('returns COMBINED source when all tiers are empty', async () => {
        ...
        const result = await orchestrator.retrieveContext('conv-1', 'user-1', [0.1], ctx)
    ...
    describe('L1 failure handling', () => {
      it('continues to L2 when L1 fails', async () => {
        const messages = [createMessage()]
        l1.getRecentMessages = vi.fn().mockResolvedValue(
          err({ kind: 'ConnectionError' as const, message: 'Redis down', context: {} })
        )
        l2.getConversationHistory = vi.fn().mockResolvedValue(ok(messages))
  ...
  describe('storeMessage', () => {
    ...
    it('continues when L1 fails', async () => {
      const message = createMessage()
      l1.storeMessage = vi.fn().mockResolvedValue(
        err({ kind: 'ConnectionError' as const, message: 'Redis error', context: {} })
      )
    ...
    it('returns error when L2 fails', async () => {
      const message = createMessage()
      l2.storeMessage = vi.fn().mockResolvedValue(
        err({ kind: 'ConnectionError' as const, message: 'DB error', context: {} })
      )
    ...
    it('continues when L4 fails', async () => {
      const message = createMessage()
      const embedding = [0.1, 0.2, 0.3]
      l4.indexMessage = vi.fn().mockResolvedValue(
        err({ kind: 'ConnectionError' as const, message: 'Qdrant error', context: {} })
      )
  ...
  describe('updateSessionState', () => {
    it('updates existing session state', async () => {
      ...
      const result = await orchestrator.updateSessionState('conv-1', { crisisLevel: 3 }, ctx)
    ...
    it('creates default state if none exists', async () => {
      ...
      const result = await orchestrator.updateSessionState('conv-1', { crisisLevel: 2 }, ctx)
    ...
    it('returns error when L1 set fails', async () => {
      l1.get = vi.fn().mockResolvedValue(ok(null))
      l1.set = vi.fn().mockResolvedValue(
        err({ kind: 'ConnectionError' as const, message: 'Redis error', context: {} })
      )
