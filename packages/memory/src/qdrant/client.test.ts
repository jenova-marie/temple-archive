...
describe('qdrant/client', () => {
  ...
  describe('createQdrantClient', () => {
    it('creates client with default URL', () => {
      ...
      expect((client as { _config: { url: string } })._config.url).toBe('http://localhost:6333')
    })

    it('creates client with custom URL', () => {
      const client = createQdrantClient({ url: 'http://qdrant.example.com:6333' })

      expect((client as { _config: { url: string } })._config.url).toBe('http://qdrant.example.com:6333')
    })
    ...
    it('uses QDRANT_URL environment variable', () => {
      ...
      expect((client as { _config: { url: string } })._config.url).toBe('http://env-qdrant:6333')
    })
    ...
    it('config URL takes precedence over env var', () => {
      ...
      expect((client as { _config: { url: string } })._config.url).toBe('http://config-qdrant:6333')
    })

    it('creates client with API key', () => {
      const client = createQdrantClient({ apiKey: 'test-api-key' })

      expect((client as { _config: { apiKey: string } })._config.apiKey).toBe('test-api-key')
    })
    ...
    it('uses QDRANT_API_KEY environment variable', () => {
      ...
      expect((client as { _config: { apiKey: string } })._config.apiKey).toBe('env-api-key')
    })

    it('creates client with custom timeout', () => {
      const client = createQdrantClient({ timeout: 60000 })

      expect((client as { _config: { timeout: number } })._config.timeout).toBe(60000)
    })

    it('uses default timeout of 30000ms', () => {
      const client = createQdrantClient()

      expect((client as { _config: { timeout: number } })._config.timeout).toBe(30000)
    })
  })
