...
import { messageIdToPointId } from '../qdrant/schema.js'
...
function createMockClient() {
  return {
    getCollections: vi.fn().mockResolvedValue({ collections: [{ name: 'messages' }] }),
...
describe('QdrantVectorStore', () => {
  ...
  describe('indexMessage', () => {
    ...
    it('handles Qdrant errors gracefully', async () => {
      const error = new Error('fetch failed: Connection refused') as Error & { code: string }
  ...
  describe('search', () => {
    ...
    it('builds correct filter for daysBack', async () => {
      ...
      const searchCall = mockClient.search.mock.calls[0]
      const filter = searchCall[1].filter
      const timestampFilter = filter.must.find((c: any) => c.key === 'timestamp')
    ...
    it('respects limit and scoreThreshold', async () => {
      ...
      await store.search(queryEmbedding, { limit: 5, scoreThreshold: 0.8 }, ctx)
  ...
  describe('collection management', () => {
    ...
    it('does not create collection if it already exists', async () => {
      mockClient.getCollections.mockResolvedValue({ collections: [{ name: 'messages' }] })
...
describe('messageIdToPointId', () => {
  it('generates valid UUID format', () => {
    ...
    expect(pointId).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/)
  })
