...
describe("redis/client", () => {
  ...
  describe("createRedisClient", () => {
    ...
    it("creates client with custom URL", () => {
      ...
      expect((client as { _url: string })._url).toBe(
    ...
    it("creates client with password", () => {
      ...
      expect(
        (client as { _options: { password: string } })._options.password,
      ).toBe("secret123");
    });
    ...
    it("creates client with username", () => {
      ...
      expect(
        (client as { _options: { username: string } })._options.username,
      ).toBe("myuser");
    });
    ...
    it("uses default command timeout of 5000ms", () => {
      ...
      expect(
        (client as { _options: { commandTimeout: number } })._options
          .commandTimeout,
      ).toBe(5000);
    });
    ...
    it("uses default maxRetriesPerRequest of 3", () => {
      ...
      expect(
        (client as { _options: { maxRetriesPerRequest: number } })._options
