...
describe("OpenAIEmbeddingProvider", () => {
  ...
  describe("constructor", () => {
    ...
    it("accepts API key from config", () => {
      delete process.env.OPENAI_API_KEY;
      expect(
        () => new OpenAIEmbeddingProvider({ apiKey: "custom-key" }),
      ).not.toThrow();
    });
  ...
  describe("embed", () => {
    ...
    it("handles rate limit errors", async () => {
      mockCreate.mockRejectedValue(
        new MockAPIError(429, "Rate limit exceeded", { "retry-after": "60" }),
      );
    ...
    it("handles input too long errors", async () => {
      mockCreate.mockRejectedValue(
        new MockAPIError(400, "maximum context length exceeded"),
      );
