...
import { DeepCrisisEvaluator } from './DeepCrisisEvaluator.js'
...
describe('DeepCrisisEvaluator', () => {
  ...
  describe('evaluate', () => {
              ...
              patterns: [
                {
                  ...
                  confidence: 0.9,
                  reasoning: 'User expresses hopelessness and desire to end pain',
                },
              ],
        ...
        content: [
          {
            ...
            text: 'Here is my analysis:\n' +
              JSON.stringify({
                crisisDetected: true,
                level: 6,
                patterns: [{ type: 'severe_distress', confidence: 0.7, reasoning: 'Elevated stress' }],
                requiresEscalation: false,
              }) +
    ...
    it('should include conversation history in prompt', async () => {
      ...
      await evaluator.evaluate('I do not know what to do anymore', history, ctx)

      // Verify API was called with history
      expect(mockCreate).toHaveBeenCalledTimes(1)
      const callArgs = mockCreate.mock.calls[0][0]
      expect(callArgs.messages[0].content).toContain('[1] user: I am struggling today')
      expect(callArgs.messages[0].content).toContain('[2] assistant: I hear you')
      expect(callArgs.messages[0].content).toContain('[3] user: Everything feels hard')
    })
  ...
  describe('configuration', () => {
    it('should use custom model when configured', async () => {
      ...
      await customEvaluator.evaluate('Test with custom model configuration', [], createTraceContext())
    ...
    it('should use custom maxTokens when configured', async () => {
      ...
      await customEvaluator.evaluate('Test with custom tokens config', [], createTraceContext())
