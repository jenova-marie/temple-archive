...
import { StubCrisisHandler } from './StubCrisisHandler.js'
import type { TraceContext, CrisisCheckResult } from '@recoverysky/types'
...
function createCrisisResult(overrides: Partial<CrisisCheckResult> = {}): CrisisCheckResult {
  return {
    level: 1,
    patterns: [],
...
describe('StubCrisisHandler', () => {
  ...
  describe('handle', () => {
    describe('low crisis levels (1-3)', () => {
      it('returns minimal response for level 1', async () => {
        ...
        const result = await handler.handle(crisisResult, 'user-1', 'conv-1', ctx)
    ...
    describe('high crisis levels (7-8)', () => {
      ...
      it('includes national and recovery resources', async () => {
        ...
        if (result.ok) {
          ...
          expect(result.value.resources!.some(r => r.contactValue === '988')).toBe(true)
        }
      })
    })
    ...
    describe('emergency crisis levels (9-10)', () => {
      it('alerts team at level 9', async () => {
        const crisisResult = createCrisisResult({ level: 9, triggerEmergency: true })
      ...
      it('alerts team at level 10', async () => {
        const crisisResult = createCrisisResult({ level: 10, triggerEmergency: true })
    ...
    describe('triggerEmergency flag', () => {
      it('sets teamAlerted based on triggerEmergency', async () => {
        const crisisWithEmergency = createCrisisResult({ level: 9, triggerEmergency: true })
        const crisisWithoutEmergency = createCrisisResult({ level: 9, triggerEmergency: false })

        const resultWith = await handler.handle(crisisWithEmergency, 'user-1', 'conv-1', ctx)
        const resultWithout = await handler.handle(crisisWithoutEmergency, 'user-1', 'conv-1', ctx)
  ...
  describe('getHandledCrises', () => {
    ...
    it('records multiple crises', async () => {
      await handler.handle(createCrisisResult({ level: 3 }), 'user-1', 'conv-1', ctx)
      await handler.handle(createCrisisResult({ level: 7 }), 'user-2', 'conv-2', ctx)
      await handler.handle(createCrisisResult({ level: 9 }), 'user-1', 'conv-3', ctx)
  ...
  describe('clear', () => {
    it('clears all handled crises', async () => {
      await handler.handle(createCrisisResult({ level: 5 }), 'user-1', 'conv-1', ctx)
      await handler.handle(createCrisisResult({ level: 8 }), 'user-2', 'conv-2', ctx)
