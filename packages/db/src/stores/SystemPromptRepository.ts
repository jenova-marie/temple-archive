...
export class SystemPromptRepository {
  ...
  async findActive(
    name: string,
  ): Promise<Result<SystemPrompt | null, SystemPromptError>> {
    try {
      const rows = await this.db
        .select()
        .from(systemPrompts)
        .where(
          and(eq(systemPrompts.name, name), eq(systemPrompts.active, true)),
        )
        .limit(1);
      ...
      return err(
        createError(
          "DatabaseError",
          `Failed to find active prompt: ${name}`,
          error,
        ),
      );
    }
  ...
  async findById(
    ...
    try {
      ...
      return err(
        createError(
          "DatabaseError",
          `Failed to find prompt by id: ${id}`,
          error,
        ),
      );
    }
  ...
  async findByName(
    name: string,
  ): Promise<Result<SystemPrompt[], SystemPromptError>> {
    try {
      const rows = await this.db
        .select()
        .from(systemPrompts)
      ...
      return err(
        createError(
          "DatabaseError",
          `Failed to find prompts by name: ${name}`,
          error,
        ),
      );
    }
  ...
  async findAllActive(): Promise<Result<SystemPrompt[], SystemPromptError>> {
    try {
      ...
      return err(
        createError(
          "DatabaseError",
          "Failed to find all active prompts",
          error,
        ),
      );
    }
   ...
   */
  async insert(
    data: Omit<SystemPrompt, "created" | "updated"> & {
      created?: string;
      updated?: string;
    },
  ): Promise<Result<SystemPrompt, SystemPromptError>> {
    try {
      ...
      return err(
        createError("DatabaseError", "Failed to insert prompt", error),
      );
    }
  }
  ...
  async update(
    id: string,
    changes: Partial<Omit<SystemPrompt, "id" | "created">>,
  ): Promise<Result<SystemPrompt | null, SystemPromptError>> {
    try {
      const [row] = await this.db
      ...
      return err(
        createError("DatabaseError", `Failed to update prompt: ${id}`, error),
      );
    }
  }
  ...
  async setActive(
    ...
    try {
      ...
      return err(
        createError(
          "DatabaseError",
          `Failed to set active prompt: ${id}`,
          error,
        ),
      );
    }
  ...
  async delete(
    ...
    try {
      ...
      return err(
        createError("DatabaseError", `Failed to delete prompt: ${id}`, error),
      );
    }
  }
}
