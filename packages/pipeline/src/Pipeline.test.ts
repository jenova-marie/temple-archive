...
import { Pipeline, type PipelineDependencies } from './Pipeline.js'
...
function createPipelineInput(overrides: Partial<PipelineInput> = {}): PipelineInput {
  return {
...
function createAgentResponse(overrides: Partial<AgentResponse> = {}): AgentResponse {
  return {
    content: 'I hear you. Recovery can be challenging, but you are doing great.',
...
function createMockDependencies(overrides: Partial<{
  ...
  const crisisHandler: ICrisisHandler = {
    handle: vi.fn().mockResolvedValue(ok({ prependMessage: null, resources: [] })),
    ...overrides.crisisHandler,
  }

  const mockMemory = {
    retrieveContext: vi.fn().mockResolvedValue(ok({
  ...
  describe('process', () => {
    ...
    describe('crisis handling', () => {
      it('triggers emergency for level 9+ crisis', async () => {
        ...
        deps = createMockDependencies({
          crisisDetector: { detect: vi.fn().mockResolvedValue(ok(emergencyCrisis)) },
          crisisHandler: {
            handle: vi.fn().mockResolvedValue(ok({
              prependMessage: 'I am concerned about your safety. Please call 988.',
        ...
        deps = createMockDependencies({
          crisisDetector: {
            detect: vi.fn().mockResolvedValue(
              err({ kind: 'DetectionError', message: 'Detector failed', context: {} })
            ),
          },
        })
        pipeline = new Pipeline(deps)
      ...
      it('continues with empty context when memory fails', async () => {
        const mockMemory = {
          retrieveContext: vi.fn().mockResolvedValue(
            err({ kind: 'RetrievalError', message: 'Memory failed', context: {} })
          ),
          storeMessage: vi.fn().mockResolvedValue(ok(undefined)),
          updateSessionState: vi.fn().mockResolvedValue(ok(undefined)),
        }
        ...
        deps = createMockDependencies({
          agent: {
            generate: vi.fn().mockResolvedValue(
              err({ kind: 'GenerationError', message: 'Agent failed', context: {} })
            ),
          },
        })
        pipeline = new Pipeline(deps)
      ...
      it('includes safety violations in result', async () => {
        deps = createMockDependencies({
          safety: {
            validate: vi.fn().mockResolvedValue(ok({
              passed: false,
              violations: [{ type: 'medical_advice', severity: 'high', description: 'Contains medical advice' }],
        ...
        if (result.ok) {
          ...
          expect(result.value.response).toBe('Please consult a healthcare provider.')
        }
      })
